...
namespace BlogLab.Web.Controllers
{
    ...
    public class PhotoController : ControllerBase
    {
        ...
        public async Task<ActionResult<List<Photo>>> GetByApplicationUserId()
        {
            ...
            var photos = await _photoRepository.GetAllByUserIdsync(applicationUserId);
        ...
        public async Task<ActionResult<int>> Delete(int photoId)
        {
            ...
            if (foundPhoto != null)
            {
                if (foundPhoto.ApplicationUserId == applicationUserId)
                {
                    ...
                    if (usedInBlog) return BadRequest("cannot remove phot as it is used");

                    var deletedResult = await _photoService.DeletePhotoAsync(foundPhoto.PublicId);
                ...
                {
                    return BadRequest("photo not uploaded by u");
                }
            }
            return BadRequest("photo no exist");
        }
    }
}
